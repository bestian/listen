  ...
  <div class="w-full h-screen flex">
    ...
    <div class="fixed bottom-16 right-6 z-50 flex flex-col space-y-3">
      ...
      <button
        ...
        :title="$t('meeting.audioSettingButton')"
      >
...
export default {
  ...
  data() {
    return {
      ...
      showJitsiTipBanner: (typeof window !== 'undefined' && window.localStorage)
        ? localStorage.getItem('listen_jitsi_tip_dismissed') !== '1'
        : true,
    };
  },
  computed: {
  ...
  methods: {
    dismissJitsiTipBanner() {
      ...
      try {
        localStorage.setItem('listen_jitsi_tip_dismissed', '1');
      } catch (e) {
    ...
    async getJwt() {
      ...
      const user_email = this.userData.email || 'guest@listen.tw';
      const isAdmin = this.userData.isAdmin || false;
      // console.log('user_id', user_id);
      // console.log('user_name', user_name);
      // console.log('user_email', user_email);
      // console.log('isAdmin', isAdmin);
      const res = await fetch(`https://listen-jaas-jwt-worker.bestian123.workers.dev/api/jitsi-token?room=listen&user_id=${user_id}&user_name=${user_name}&user_email=${user_email}&user_moderator=${isAdmin}`);
